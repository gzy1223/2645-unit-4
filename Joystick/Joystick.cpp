...
Joystick::Joystick(PinName vertPin,PinName horizPin,PinName SW_Pin){
    vert = new AnalogIn(vertPin);
    horiz = new AnalogIn(horizPin);
    Button_SW = new AnalogIn(SW_Pin);
}
...
bool Joystick::Button_press()
{
    if(Button_SW->read()>0.9)//only value bigger than 0.9, so if button not pressed it will hold the 0 
    {
        return true;
    }
    else {

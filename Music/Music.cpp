...
void Music::play_note(int frequency)
{
...
void Music::start_music()
{
  ...
  NOTE_A4, 2, 
                ...
                for(int i = 0; i < (sizeof(melody)/sizeof(melody[0])); i++){         //iterate through the C_major_scale array
                play_note(melody[i]);    //pass the note at position C_major_scale[i] to function
            }

            buzzer.pulsewidth_us(0);            //turn off buzzer
}
void Music::block_music()
{
    int melody[] = {
